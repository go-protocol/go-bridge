...
contract Bridge is BridgeAdmin, Pausable {
    ...
    constructor(address[] memory _owners, uint256 _ownerRequired) {
    ...
    function withdrawNative(
        ...
        string memory proof,
        bytes32 taskHash
    ) public onlyOperator whenNotPaused positiveValue(value) returns (bool) {
    ...
    function transferToken(
        ...
        atoken.transfer(to, value);
    }

    function setDepositSelector(
        address token,
    ...
    function depositTokenLogic(
        ...
        require(status && (returnedData.length == 0 || abi.decode(returnedData, (bool))), " transfer failed");
        return true;
    }

    function withdrawTokenLogic(
        address token,
        address _to,
        uint256 _value
    ) internal returns (bool) {
